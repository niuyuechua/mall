    ...
    public function wxEvent(){
        ...
        if($msg_type=='event'){
            ...
            if($event=='CLICK' && $event_key=='function declaration'){
                ReplyModel::where(['menu_key'=>$event_key])->encrement('num');
                $str="Hello";
                $this->sendTextMsg($openid,$str);
            }elseif($event=='VIEW'){
                ReplyModel::where(['menu_key'=>'https://pvp.qq.com/'])->encrement('num');
            }elseif($event=='pic_weixin'){
                ReplyModel::where(['menu_key'=>'sendPic'])->encrement('num');
                $str="发送图片";
                $this->sendTextMsg($openid,$str);
            }
        }
